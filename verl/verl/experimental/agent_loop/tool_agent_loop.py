...
# [W5 PRM-Lite] 导入 assistant content 和 policy-error 记录函数
...
def _merge_initial_observation(messages: list[dict[str, Any]], initial_observation: str) -> None:
    """Append the reset observation, or validate an existing initial user message."""
...
class AgentData:
    ...
    def invalidate(self, stage: str, reason: str) -> None:
        """Exclude infrastructure-failed trajectories from every training objective."""
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        try:
            if agent_data.interaction is not None:
                # finalize_interaction is idempotent and must run even if start/get-initial fails.
                ...
                if initial_observation is not None:
                    if not str(initial_observation):
                        raise RuntimeError("Interaction returned an empty initial observation")
                    _merge_initial_observation(messages, str(initial_observation))
